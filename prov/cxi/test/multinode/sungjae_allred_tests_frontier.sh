...
#SBATCH -p batch
#SBATCH --exclusive
#SBATCH -o "%j-CXI-allred-tests.txt"
#SBATCH -J "CXI test"
...
mpirun -np $np -ppn $ppn \
    ...
    -genv FI_CXI_HWCOLL_ADDRS_PER_JOB=$np \
    -genv FI_CXI_HWCOLL_MIN_NODES=2 \
    -genv FI_CXI_BENCHMARK_MAX_CNT=$((128*1024)) \
    -genv FI_CXI_BENCHMARK_CNT=$((128*1024)) \
    -genv FI_CXI_BENCHMARK_MAX_ITER=$numiters \
    ./test_coll -v -V -N 1 -n $numiters -t13,14,15 > $outfile 2>&1
date
set +x
...
mpirun -np $np -ppn $ppn \
    ...
    -genv FI_CXI_BENCHMARK_CNT=$((128*1024)) \
    -genv FI_CXI_BENCHMARK_MAX_ITER=$numiters \
    ./test_coll -v -V -N 1 -n $numiters -M -t13,14,15 -n $numiters > $outfile 2>&1
date
set +x
