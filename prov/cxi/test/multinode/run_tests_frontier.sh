...
#SBATCH -p batch
#SBATCH --exclusive
#SBATCH -o "%j-Frontier-CXI-tests.txt"
#SBATCH -J "CXI test"
...
mkdir $exp
outfile=$exp/$SLURM_JOBID-Frontier-$SLURM_NNODES-nodes-$ppn-ppn-test-cxi-coll-unicast.txt
rm $outfile
set -x
date
mpirun -np $np -ppn $ppn \
...
outfile=$exp/$SLURM_JOBID-Frontier-$SLURM_NNODES-nodes-$ppn-ppn-test-cxi-coll-multicast.txt
rm $outfile
set -x
date
mpirun -np $np -ppn $ppn \
